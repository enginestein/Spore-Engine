...
from ..core.color import Color, Gradient, WHITE, DIM
...
def typewriter_text(canvas: Canvas, x: int, y: int, text: str, t: float,
                    chars_per_sec: float = 10, fg: Optional[Color] = None,
                    cursor: bool = True):
    color = fg or WHITE
...
def sine_text(canvas: Canvas, x: int, y: int, text: str, t: float,
              amplitude: float = 2, frequency: float = 2, fg: Optional[Color] = None):
    color = fg or WHITE
    for i, ch in enumerate(text):
...
def scroll_text(canvas: Canvas, text: str, t: float,
                height: int = 0, speed: float = 2, fg: Optional[Color] = None,
...
def matrix_code_rain(canvas: Canvas, t: float, fg: Optional[Color] = None):
...
def wave_text(canvas: Canvas, x: int, y: int, text: str, t: float,
              amplitude: float = 3, fg: Optional[Color] = None):
    color = fg or WHITE
    chars = list(text)
    n = len(chars)
    for i in range(n):
