...
class PerlinNoise:
    ...
    @staticmethod
    def _grad(hash: int, x: float, y: float = 0, z: float = 0) -> float:
        h = hash & 15
        u = x if h < 8 else y
    ...
    def fbm2(self, x: float, y: float, octaves: int = 4, lacunarity: float = 2.0, gain: float = 0.5) -> float:
    ...
    def noise2d_array(self, w: int, h: int, scale: float = 1.0,
                      ox: float = 0, oy: float = 0) -> list[list[float]]:
        result = [[0.0] * w for _ in range(h)]
        for y in range(h):
            for x in range(w):
                result[y][x] = self.noise2((x + ox) * scale, (y + oy) * scale)
        return result

    def fbm2d_array(self, w: int, h: int, scale: float = 1.0,
                    ox: float = 0, oy: float = 0, octaves: int = 4) -> list[list[float]]:
        result = [[0.0] * w for _ in range(h)]
        for y in range(h):
            for x in range(w):
                result[y][x] = self.fbm2((x + ox) * scale, (y + oy) * scale, octaves)
        return result
...
class ValueNoise:
    ...
    def noise2(self, x: float, y: float) -> float:
        xi, yi = int(math.floor(x)) & 511, int(math.floor(y)) & 511
        xf, yf = x - math.floor(x), y - math.floor(y)
        u, v = xf * xf * (3 - 2 * xf), yf * yf * (3 - 2 * yf)
        a = self.table[xi] + yi
        b = self.table[xi + 1] + yi
        v00, v10 = self.table[a & 511], self.table[b & 511]
        v01, v11 = self.table[(a + 1) & 511], self.table[(b + 1) & 511]
...
class WorleyNoise:
    ...
    def fbm2(self, x: float, y: float, octaves: int = 4, lacunarity: float = 2.0, gain: float = 0.5) -> float:
        ...
        for _ in range(octaves):
            value += amp * self.noise2(x * freq, y * freq)
            amp *= gain
            freq *= lacunarity
        return value

    def noise2d_array(self, w: int, h: int, scale: float = 1.0, ox: float = 0, oy: float = 0) -> list[list[float]]:
        return [[self.noise2((x + ox) * scale, (y + oy) * scale) for x in range(w)] for y in range(h)]
...
class OpenSimplexNoise:
    ...
    def noise2(self, x: float, y: float) -> float:
        ...
        n = 0.0
        for (di, dj, dx, dy) in [(0, 0, x0, y0), (i1, j1, x1, y1), (1, 1, x2, y2)]:
            t2 = 0.5 - dx * dx - dy * dy
            if t2 > 0:
